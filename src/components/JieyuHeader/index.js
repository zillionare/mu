            ...
            <Row style={{ align: 'middle' }}>
                <Col offset={5} span={2} className='menu_home'>
                    <div className='menu_home1'>
                        ...
                        <div className='home'><Link to={'/'}>解语科技</Link></div>
                    </div>
                </Col>
                {
                    navs.map(item => {
                        return <Col span={1} key={item.title} className='menu_item'><Link to={item.path}>{item.title}</Link></Col>
                    })
                }
                <Col className='menu_item' offset={7} span={1}>
                    <Link to='/' id='login'>登录</Link>
                </Col>
            </Row>
        </div>
